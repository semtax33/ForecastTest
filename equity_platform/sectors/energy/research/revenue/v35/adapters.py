...
from equity_platform.documents import CanonicalDocument, DocumentMetadata, DocumentTable
from equity_platform.ir import SourceRef
from equity_platform.parsing import compile_rule_file, execute_rules
from equity_platform.paths import PROJECT_ROOT
from equity_platform.sectors.energy.parsing import load_ir_tables

from .taxonomy import all_tickers, group_for_ticker


EP_V35_ACTUAL_RULES = compile_rule_file(
    PROJECT_ROOT / "configs/parser_rules/energy/ep_v35_actuals.arc"
)
...
def _table_rows(path: Path) -> list[list[list[str]]]:
    try:
        release_date = pd.Timestamp(path.name[:10])
    except ValueError:
        release_date = pd.Timestamp("1970-01-01")
    try:
        return load_ir_tables(
            path,
            ticker=path.parent.name,
            report_period=str(release_date.to_period("Q") - 1),
            available_at=release_date.isoformat(),
            source_uri=str(path),
        )
...
def _actual_layout_facts(
    ...
    """Resolve issuer-specific row layouts through the declarative parser DSL."""

    applicable_rules = tuple(
        rule
        for rule in EP_V35_ACTUAL_RULES
        if not rule.entities or ticker in rule.entities
    )
    if not applicable_rules:
        return {}
    content = path.read_bytes() if path.exists() else str(path).encode("utf-8")
    document = CanonicalDocument(
        metadata=DocumentMetadata(
            entity=ticker,
            source_kind="COMPANY_IR_SEC",
            document_kind="EARNINGS_RELEASE",
            available_at=filing_date.isoformat(),
            report_period=quarter,
        ),
        source=SourceRef(
            source_type="COMPANY_IR_SEC",
            uri=source_url,
            local_path=str(path),
            sha256=sha256(content).hexdigest(),
            available_at=filing_date.isoformat(),
        ),
        text=" ".join(cell for table in tables for row in table for cell in row),
        tables=tuple(
            DocumentTable(
                resolved_table_index=table_index,
                cells=tuple(tuple(str(cell) for cell in row) for row in table),
                source_uri=source_url,
                source_description="COMPANY_IR_OR_SEC_FILING",
                source_table_index=table_index,
                source_row_indices=tuple(range(len(table))),
            )
    ...
    return {
        execution.fact.metric: float(execution.fact.value)
        for execution in execute_rules(document, applicable_rules)
        if execution.status == "EMITTED" and execution.fact is not None
    }
...
def _extract_actual_from_file(
    ...
    source_tables = tables if tables is not None else _table_rows(path)
    layout_facts = _actual_layout_facts(
        ticker, path, source_tables, filing_date, quarter, source_url
    )
    production.update(
        {
            "gas": layout_facts.get("ep_actual_gas_mmcfd", production["gas"]),
            "oil": layout_facts.get("ep_actual_oil_mbpd", production["oil"]),
            "ngl": layout_facts.get("ep_actual_ngl_mbpd", production["ngl"]),
            "total": layout_facts.get("ep_actual_total_mboed", production["total"]),
        }
    )
    if np.isfinite(production["total"]):
        raw_total_value = production["total"] * 6.0
