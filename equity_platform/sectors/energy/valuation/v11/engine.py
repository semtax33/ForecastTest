...
from equity_platform.valuation_kernel import DcfAssumptions, enterprise_value
...
def project_dcf_v11(
    *,
    ...
    frame, summary = enterprise_value(
        DcfAssumptions(
            ticker="ENERGY_V1_1",
            scenario="LEGACY_COMPATIBILITY",
            base_revenue_usd=float(base_revenue),
            near_term_growth_pct=float(growth_pct),
            terminal_growth_pct=float(terminal_growth_pct),
            initial_margin_pct=float(margin_pct),
            terminal_margin_pct=float(margin_pct),
            tax_rate_pct=float(tax_rate_pct),
            initial_roic_pct=float(roic_pct),
            terminal_roic_pct=float(roic_pct),
    ...
    for row in frame.to_dict("records"):
        reinvestment_rate_pct = float(row["reinvestment_rate_pct"])
        projection: dict[str, float | str] = {
            "forecast_year": int(row["forecast_year_index"]),
            "revenue_usd": float(row["revenue_usd"]),
            "growth_pct": float(row["revenue_growth_pct"]),
            "operating_margin_pct": float(row["operating_margin_pct"]),
            "ebit_usd": float(row["ebit_usd"]),
            "nopat_usd": float(row["nopat_usd"]),
            "forecast_normalized_roic_pct": float(row["roic_pct"]),
            "reinvestment_rate_pct": reinvestment_rate_pct,
            "reinvestment_usd": float(row["reinvestment_usd"]),
            "fcff_usd": float(row["fcff_usd"]),
            "fundamental_growth_reconstructed_pct": (
                reinvestment_rate_pct * float(row["roic_pct"]) / 100.0
            ),
            ...
            "pv_fcff_usd": float(row["pv_fcff_usd"]),
        }
        for name in (
            "terminal_reinvestment_rate_pct",
            "terminal_fcff_usd",
            "terminal_value_usd",
            "pv_terminal_value_usd",
        ):
            if name in row and pd.notna(row[name]):
                projection[name] = float(row[name])
