...
from ..v279 import extract_text_kpis_v279
from .router import route_document_blocks_v280
from .semantics import augment_candidates_v280, recover_v280_frames, resolve_frame_conflicts_v280


def extract_text_kpis_v280(document: CanonicalDocument) -> V26ExtractionResult:
    prior = extract_text_kpis_v279(document)
    candidates = augment_candidates_v280(document, prior.candidates)
    routes = route_document_blocks_v280(document)
    allowed_spans = {
    ...
    recovered = recover_v280_frames(document, candidates, allowed_spans)
    frames = resolve_frame_conflicts_v280(existing, recovered)
    extraction = TextExtractionResult(
        ...
        backend_name="V280_SPACY_HIERARCHICAL_CONTEXT_PIPELINE",
    )
    bindings = tuple(
        RouteBindingEvidence(
            route=(
                EvidenceRoute.ADJUDICATED_DIRECT
                if frame.rule_id.startswith(tuple(f"v{version}." for version in range(264, 281)))
            ...
            verifier=frame.verified_by or "V280_UNKNOWN",
        )
        for frame in frames
    )
