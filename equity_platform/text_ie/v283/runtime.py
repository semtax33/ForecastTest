...
from ..v282 import extract_text_kpis_v282
...
def extract_text_kpis_v283(document: CanonicalDocument) -> V26ExtractionResult:
    prior = extract_text_kpis_v282(document)
    candidates = augment_candidates_v283(document, prior.candidates)
    routes = route_document_blocks_v283(document)
    allowed_spans = {
    ...
    recovered = recover_v283_frames(document, candidates, allowed_spans)
    frames = resolve_frame_conflicts_v283(existing, recovered)
    extraction = TextExtractionResult(
        ...
        backend_name="V283_SPACY_BOUNDED_UPPER_CONTEXT_PIPELINE",
    )
    bindings = tuple(
        RouteBindingEvidence(
            route=(
                ...
                if frame.rule_id.startswith(
                    tuple(f"v{version}." for version in range(264, 284))
                )
            ...
            verifier=frame.verified_by or "V283_UNKNOWN",
        )
        for frame in frames
    )
