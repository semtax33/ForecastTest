...
from ..v289 import extract_text_kpis_v289
from .router import route_document_blocks_v290
from .semantics import augment_candidates_v290, recover_v290_frames, resolve_frame_conflicts_v290


def extract_text_kpis_v290(
    document: CanonicalDocument,
    *,
    telemetry_observer=None,
) -> V26ExtractionResult:
    prior = extract_text_kpis_v289(document)
    candidates = augment_candidates_v290(document, prior.candidates)
    routes = route_document_blocks_v290(document)
    allowed_spans = {
    ...
    recovered, adjudicated_spans = recover_v290_frames(
        document,
        candidates,
        allowed_spans,
        telemetry_observer,
    )
    frames = resolve_frame_conflicts_v290(existing, recovered, adjudicated_spans)
    extraction = TextExtractionResult(
        ...
        backend_name="V290_SPACY_SEMANTIC_ROLE_GRAPH_PIPELINE",
    )
    bindings = tuple(
        RouteBindingEvidence(
            ...
            verifier=frame.verified_by or "V290_UNKNOWN",
        )
        for frame in frames
    )
