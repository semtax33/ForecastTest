...
from ..v271 import extract_text_kpis_v271
from .router import route_document_blocks_v272
from .semantics import (
    augment_candidates_v272,
    recover_v272_frames,
    resolve_frame_conflicts_v272,
    semantic_backend_v272,
)


def extract_text_kpis_v272(document: CanonicalDocument) -> V26ExtractionResult:
    prior = extract_text_kpis_v271(document)
    backend = semantic_backend_v272()
    candidates = augment_candidates_v272(document, prior.candidates, backend)
    routes = route_document_blocks_v272(document)
    allowed_spans = {
    ...
    recovered = recover_v272_frames(document, candidates, allowed_spans, backend=backend)
    frames = resolve_frame_conflicts_v272(document, existing, recovered, backend)
    extraction = TextExtractionResult(
        ...
        backend_name="V272_SPACY_SEMANTIC_OWNERSHIP_PIPELINE",
    )
    bindings = tuple(
        RouteBindingEvidence(
            ...
            verifier=frame.verified_by or "V272_UNKNOWN",
        )
        for frame in frames
    )
