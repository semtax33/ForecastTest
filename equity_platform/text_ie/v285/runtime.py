...
from ..v284 import extract_text_kpis_v284
from .router import route_document_blocks_v285
from .semantics import augment_candidates_v285, recover_v285_frames, resolve_frame_conflicts_v285


def extract_text_kpis_v285(document: CanonicalDocument) -> V26ExtractionResult:
    prior = extract_text_kpis_v284(document)
    candidates = augment_candidates_v285(document, prior.candidates)
    routes = route_document_blocks_v285(document)
    allowed_spans = {
        (route.char_start, route.char_end)
        for route in routes
        if route.char_start is not None and route.char_end is not None and route.route not in {BlockRoute.FLATTENED_TABLE, BlockRoute.MIXED}
    }
    existing = tuple(
        frame
        for frame in prior.extraction.frames
        if any(frame.source_span.char_start >= start and frame.source_span.char_end <= end for start, end in allowed_spans)
    )
    recovered = recover_v285_frames(document, candidates, allowed_spans)
    frames = resolve_frame_conflicts_v285(existing, recovered)
    extraction = TextExtractionResult(
        ...
        backend_name="V285_SPACY_GENERALIZED_FINANCIAL_ROLE_PIPELINE",
    )
    bindings = tuple(
        RouteBindingEvidence(
            route=EvidenceRoute.ADJUDICATED_DIRECT if frame.rule_id.startswith(tuple(f"v{version}." for version in range(264, 286))) else EvidenceRoute.TEXT_BINDING,
            frame=frame,
            upstream_candidate_id=str(frame.context_trace.get("candidate_id")) if frame.context_trace.get("candidate_id") else None,
            verifier=frame.verified_by or "V285_UNKNOWN",
        )
        for frame in frames
    )
