...
from ..v274 import extract_text_kpis_v274
...
def extract_text_kpis_v275(document: CanonicalDocument) -> V26ExtractionResult:
    prior = extract_text_kpis_v274(document)
    backend = semantic_backend_v275()
    candidates = augment_candidates_v275(document, prior.candidates, backend)
    ...
    recovered = recover_v275_frames(document, candidates, allowed_spans, backend=backend)
    frames = resolve_frame_conflicts_v275(document, existing, recovered, backend)
    extraction = TextExtractionResult(
        ...
        backend_name="V275_SPACY_BOUNDED_DOCUMENT_CONTEXT_PIPELINE",
    )
    bindings = tuple(
        RouteBindingEvidence(
            route=(
                EvidenceRoute.ADJUDICATED_DIRECT
                if frame.rule_id.startswith(tuple(f"v{version}." for version in range(264, 276)))
            ...
            verifier=frame.verified_by or "V275_UNKNOWN",
        )
        for frame in frames
    )
