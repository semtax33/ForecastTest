...
from ..v268 import extract_text_kpis_v268
...
def extract_text_kpis_v269(document: CanonicalDocument) -> V26ExtractionResult:
    prior = extract_text_kpis_v268(document)
    backend = semantic_backend_v269()
    candidates = augment_candidates_v269(document, prior.candidates, backend)
    routes = route_document_blocks_v269(document)
    allowed_spans = {
    ...
    recovered = recover_v269_frames(document, candidates, allowed_spans, backend=backend)
    frames = resolve_frame_conflicts_v269(existing, recovered, backend)
    extraction = TextExtractionResult(
        ...
        backend_name="V269_SPACY_SEMANTIC_OWNERSHIP_PIPELINE",
    )
    bindings = tuple(
        RouteBindingEvidence(
            ...
            verifier=frame.verified_by or "V269_UNKNOWN",
        )
        for frame in frames
    )
