...
from ..v277 import extract_text_kpis_v277
from .router import route_document_blocks_v278
from .semantics import (
    augment_candidates_v278,
    recover_v278_frames,
    resolve_frame_conflicts_v278,
)


def extract_text_kpis_v278(document: CanonicalDocument) -> V26ExtractionResult:
    prior = extract_text_kpis_v277(document)
    candidates = augment_candidates_v278(document, prior.candidates)
    routes = route_document_blocks_v278(document)
    allowed_spans = {
    ...
    recovered = recover_v278_frames(document, candidates, allowed_spans)
    frames = resolve_frame_conflicts_v278(document, existing, recovered)
    extraction = TextExtractionResult(
        ...
        backend_name="V278_SPACY_SEMANTIC_LAW_PIPELINE",
    )
    bindings = tuple(
        RouteBindingEvidence(
            route=(
                EvidenceRoute.ADJUDICATED_DIRECT
                if frame.rule_id.startswith(
                    tuple(f"v{version}." for version in range(264, 279))
                )
            ...
            verifier=frame.verified_by or "V278_UNKNOWN",
        )
        for frame in frames
    )
