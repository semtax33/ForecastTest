...
from ..v270 import extract_text_kpis_v270
from .router import route_document_blocks_v271
from .semantics import (
    augment_candidates_v271,
    recover_v271_frames,
    resolve_frame_conflicts_v271,
    semantic_backend_v271,
)


def extract_text_kpis_v271(document: CanonicalDocument) -> V26ExtractionResult:
    prior = extract_text_kpis_v270(document)
    backend = semantic_backend_v271()
    candidates = augment_candidates_v271(document, prior.candidates, backend)
    routes = route_document_blocks_v271(document)
    allowed_spans = {
    ...
    recovered = recover_v271_frames(document, candidates, allowed_spans, backend=backend)
    frames = resolve_frame_conflicts_v271(existing, recovered, backend)
    extraction = TextExtractionResult(
        ...
        backend_name="V271_SPACY_SEMANTIC_OWNERSHIP_PIPELINE",
    )
    bindings = tuple(
        RouteBindingEvidence(
            ...
            verifier=frame.verified_by or "V271_UNKNOWN",
        )
        for frame in frames
    )
