...
from ..v269 import extract_text_kpis_v269
...
def extract_text_kpis_v270(document: CanonicalDocument) -> V26ExtractionResult:
    prior = extract_text_kpis_v269(document)
    backend = semantic_backend_v270()
    candidates = augment_candidates_v270(document, prior.candidates, backend)
    routes = route_document_blocks_v270(document)
    allowed_spans = {
    ...
    recovered = recover_v270_frames(document, candidates, allowed_spans, backend=backend)
    frames = resolve_frame_conflicts_v270(existing, recovered, backend)
    extraction = TextExtractionResult(
        ...
        backend_name="V270_SPACY_SEMANTIC_OWNERSHIP_PIPELINE",
    )
    bindings = tuple(
        RouteBindingEvidence(
            ...
            verifier=frame.verified_by or "V270_UNKNOWN",
        )
        for frame in frames
    )
