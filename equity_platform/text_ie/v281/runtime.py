...
from ..v280 import extract_text_kpis_v280
from .router import route_document_blocks_v281
from .semantics import augment_candidates_v281, recover_v281_frames, resolve_frame_conflicts_v281


def extract_text_kpis_v281(document: CanonicalDocument) -> V26ExtractionResult:
    prior = extract_text_kpis_v280(document)
    candidates = augment_candidates_v281(document, prior.candidates)
    routes = route_document_blocks_v281(document)
    allowed_spans = {
    ...
    recovered = recover_v281_frames(document, candidates, allowed_spans)
    frames = resolve_frame_conflicts_v281(existing, recovered)
    extraction = TextExtractionResult(
        ...
        backend_name="V281_SPACY_HIERARCHICAL_CONTEXT_PIPELINE",
    )
    bindings = tuple(
        RouteBindingEvidence(
            route=EvidenceRoute.ADJUDICATED_DIRECT if frame.rule_id.startswith(tuple(f"v{version}." for version in range(264, 282))) else EvidenceRoute.TEXT_BINDING,
            frame=frame,
            upstream_candidate_id=str(frame.context_trace.get("candidate_id")) if frame.context_trace.get("candidate_id") else None,
            verifier=frame.verified_by or "V281_UNKNOWN",
        )
        for frame in frames
    )
