...
from ..v261 import extract_text_kpis_v261
from .router import route_document_blocks_v262
from .semantics import (
    augment_candidates_v262,
    recover_v262_frames,
    safe_existing_frame_v262,
)


def extract_text_kpis_v262(document: CanonicalDocument) -> V26ExtractionResult:
    prior = extract_text_kpis_v261(document)
    routes = route_document_blocks_v262(document)
    candidates = augment_candidates_v262(document, prior.candidates)
    allowed_spans = {
    ...
    extraction = TextExtractionResult(
        ...
        backend_name="V262_ROUTE_AND_RANGE_GUARDED_BINDING",
    )
    bindings = tuple(
        RouteBindingEvidence(
            route=(
                EvidenceRoute.ADJUDICATED_DIRECT
                if str(frame.context_trace.get("candidate_id", "")).startswith(
                    ("v261.direct.", "v262.direct.")
                )
                or frame.rule_id.startswith(("v261.", "v262."))
            ...
            verifier=frame.verified_by or "V262_UNKNOWN",
        )
        for frame in frames
    )
    ...
    return V26ExtractionResult(
        ...
        rejections=tuple(
            item for item in prior.rejections if item.candidate_id not in bound_ids
        ),
    )
