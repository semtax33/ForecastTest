...
from ..v272 import extract_text_kpis_v272
from .router import route_document_blocks_v273
from .semantics import (
    augment_candidates_v273,
    recover_v273_frames,
    resolve_frame_conflicts_v273,
    semantic_backend_v273,
)


def extract_text_kpis_v273(document: CanonicalDocument) -> V26ExtractionResult:
    prior = extract_text_kpis_v272(document)
    backend = semantic_backend_v273()
    candidates = augment_candidates_v273(document, prior.candidates, backend)
    routes = route_document_blocks_v273(document)
    allowed_spans = {
    ...
    recovered = recover_v273_frames(document, candidates, allowed_spans, backend=backend)
    frames = resolve_frame_conflicts_v273(document, existing, recovered, backend)
    extraction = TextExtractionResult(
        ...
        backend_name="V273_SPACY_SEMANTIC_CLAUSE_PIPELINE",
    )
    bindings = tuple(
        RouteBindingEvidence(
            ...
            verifier=frame.verified_by or "V273_UNKNOWN",
        )
        for frame in frames
    )
