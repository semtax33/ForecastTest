...
from ..v281 import extract_text_kpis_v281
from .router import route_document_blocks_v282
from .semantics import augment_candidates_v282, recover_v282_frames, resolve_frame_conflicts_v282


def extract_text_kpis_v282(document: CanonicalDocument) -> V26ExtractionResult:
    prior = extract_text_kpis_v281(document)
    candidates = augment_candidates_v282(document, prior.candidates)
    routes = route_document_blocks_v282(document)
    allowed_spans = {
    ...
    recovered = recover_v282_frames(document, candidates, allowed_spans)
    frames = resolve_frame_conflicts_v282(existing, recovered)
    extraction = TextExtractionResult(
        ...
        backend_name="V282_SPACY_UPPER_CONTEXT_PIPELINE",
    )
    bindings = tuple(
        RouteBindingEvidence(
            route=EvidenceRoute.ADJUDICATED_DIRECT if frame.rule_id.startswith(tuple(f"v{version}." for version in range(264, 283))) else EvidenceRoute.TEXT_BINDING,
            frame=frame,
            upstream_candidate_id=str(frame.context_trace.get("candidate_id")) if frame.context_trace.get("candidate_id") else None,
            verifier=frame.verified_by or "V282_UNKNOWN",
        )
        for frame in frames
    )
