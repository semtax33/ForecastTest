...
from ..v283 import extract_text_kpis_v283
from .router import route_document_blocks_v284
from .semantics import augment_candidates_v284, recover_v284_frames, resolve_frame_conflicts_v284


def extract_text_kpis_v284(document: CanonicalDocument) -> V26ExtractionResult:
    prior = extract_text_kpis_v283(document)
    candidates = augment_candidates_v284(document, prior.candidates)
    routes = route_document_blocks_v284(document)
    allowed_spans = {
    ...
    recovered = recover_v284_frames(document, candidates, allowed_spans)
    frames = resolve_frame_conflicts_v284(existing, recovered)
    extraction = TextExtractionResult(
        ...
        backend_name="V284_SPACY_ECONOMIC_ROLE_PIPELINE",
    )
    bindings = tuple(
        RouteBindingEvidence(
            route=(
                EvidenceRoute.ADJUDICATED_DIRECT
                if frame.rule_id.startswith(tuple(f"v{version}." for version in range(264, 285)))
            ...
            verifier=frame.verified_by or "V284_UNKNOWN",
        )
        for frame in frames
    )
