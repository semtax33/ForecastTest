...
from ..v267 import extract_text_kpis_v267
from .router import route_document_blocks_v268
from .semantics import (
    augment_candidates_v268,
    recover_v268_frames,
    resolve_frame_conflicts_v268,
    semantic_backend_v268,
)


def extract_text_kpis_v268(document: CanonicalDocument) -> V26ExtractionResult:
    prior = extract_text_kpis_v267(document)
    backend = semantic_backend_v268()
    candidates = augment_candidates_v268(document, prior.candidates, backend)
    routes = route_document_blocks_v268(document)
    allowed_spans = {
    ...
    recovered = recover_v268_frames(
        document, candidates, allowed_spans, backend=backend
    )
    frames = resolve_frame_conflicts_v268(existing, recovered, backend)
    extraction = TextExtractionResult(
        ...
        backend_name="V268_SPACY_SEMANTIC_OWNERSHIP_PIPELINE",
    )
    bindings = tuple(
        RouteBindingEvidence(
            route=(
                EvidenceRoute.ADJUDICATED_DIRECT
                if frame.rule_id.startswith(("v264.", "v265.", "v266.", "v267.", "v268."))
            ...
            verifier=frame.verified_by or "V268_UNKNOWN",
        )
        for frame in frames
    )
