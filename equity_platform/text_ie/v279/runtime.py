...
from ..v278 import extract_text_kpis_v278
from .router import route_document_blocks_v279
from .semantics import recover_v279_frames, resolve_frame_conflicts_v279


def extract_text_kpis_v279(document: CanonicalDocument) -> V26ExtractionResult:
    prior = extract_text_kpis_v278(document)
    candidates = prior.candidates
    routes = route_document_blocks_v279(document)
    allowed_spans = {
    ...
    recovered = recover_v279_frames(document, candidates, allowed_spans)
    frames = resolve_frame_conflicts_v279(existing, recovered)
    extraction = TextExtractionResult(
        ...
        backend_name="V279_SPACY_SEMANTIC_OWNERSHIP_PIPELINE",
    )
    bindings = tuple(
        RouteBindingEvidence(
            route=(
                ...
                if frame.rule_id.startswith(
                    tuple(f"v{version}." for version in range(264, 280))
                )
            ...
            verifier=frame.verified_by or "V279_UNKNOWN",
        )
        for frame in frames
    )
