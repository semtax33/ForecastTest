...
ROOT = PROJECT_ROOT
CONFIG = ROOT / "configs/industrials_v6_gd.toml"
...
def main() -> int:
    ...
    parent_before = verify_hii_v54_margin_mechanism_research(ROOT)
    ir = build_gd_ir_evidence(Path(config["ir_root"]), pd.Timestamp(config["as_of_date"]))
    sec = build_gd_sec_evidence(
        project_root=ROOT,
    ...
    prospective_origin = pd.DataFrame(
        [
            {
                "period": "2026Q3",
                "forecast_as_of": "2026-07-29",
                "actual_available_at": "2099-12-31",
    ...
    industry = build_gd_industry_evidence(
    ...
    forecast = build_gd_forecast_research(
    ...
    prospective = build_gd_prospective_segment_forecast(
    ...
    research = build_gd_margin_roic_reinvestment_research(
    ...
    cross_detail, cross_summary = _cross_company(forecast)
    ...
    fixed_oos = bool(
        ...
        == 4 * int(config["expected_oos_quarters_per_segment"])
