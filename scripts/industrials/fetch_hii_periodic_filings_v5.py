...
CIK_PADDED = "0001501585"
CIK_ARCHIVE = "1501585"
START_DATE = "2020-01-01"
CUTOFF_DATE = "2026-09-03"
FORMS = {"10-K", "10-Q"}
ROOT = PROJECT_ROOT / "data-lake/bronze/industrials/v5/sec/hii"
...
def main() -> int:
    user_agent = os.environ.get(
        "SEC_USER_AGENT", "ForecastTest HII A&D research contact@example.com"
    )
    ...
    submissions_dir = submissions_path.parent
    for older in payload["filings"].get("files", []):
        older_path = submissions_dir / older["name"]
        older_url = f"https://data.sec.gov/submissions/{older['name']}"
        if not older_path.exists():
            _download(older_url, older_path, user_agent)
    ...
    for filings in filing_sets:
        for index, form in enumerate(filings["form"]):
            ...
            rows.append({
                "ticker": "HII", "cik": CIK_PADDED, "form": form,
                "filing_date": filing_date, "report_date": filings["reportDate"][index],
                "accession_number": accession, "primary_document": document,
                "source_url": source_url,
                "local_path": local_path.relative_to(PROJECT_ROOT).as_posix(),
                "sha256": _sha256(local_path), "size_bytes": local_path.stat().st_size,
                "content_type": "text/html", "pdf_parsing_used": False,
            })
    rows.sort(key=lambda row: (str(row["filing_date"]), str(row["form"])))
    manifest = {
        "dataset": "HII_PERIODIC_FILINGS_INDUSTRIALS_V5", "source_authority": "SEC_EDGAR",
        "submissions_url": SUBMISSIONS_URL, "start_date": START_DATE, "cutoff_date": CUTOFF_DATE,
        "forms": sorted(FORMS), "filing_count": len(rows),
        "form_counts": {form: sum(row["form"] == form for row in rows) for form in sorted(FORMS)},
        "pdf_parsing_used": False, "files": rows,
    }
