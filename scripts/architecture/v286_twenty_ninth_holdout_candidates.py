...
from equity_platform.text_ie.v286.router import route_document_blocks_v286
from equity_platform.text_ie.v286.semantics import augment_candidates_v286
from scripts.architecture.parser_holdout import build_candidate_pool, select_candidate_blocks
import scripts.architecture.v285_twenty_eighth_holdout_candidates as parent


CONFIG = PROJECT_ROOT / "configs/certification/platform_v286_twenty_ninth_holdout.toml"
OUTPUT = PROJECT_ROOT / "output/platform_v2_8_6_strict_adjudication"
CANDIDATES = OUTPUT / "twenty_ninth_holdout_candidates.csv"
ARCANA_IR = Path("D:/Programming/python_example/Arcana/data-lake/bronze/sec/fillings/ir")
V286_SNAPSHOT = {
    "semantics": PROJECT_ROOT / "equity_platform/text_ie/v286/semantics.py",
    "runtime": PROJECT_ROOT / "equity_platform/text_ie/v286/runtime.py",
    "router": PROJECT_ROOT / "equity_platform/text_ie/v286/router.py",
    "init": PROJECT_ROOT / "equity_platform/text_ie/v286/__init__.py",
    "rules": PROJECT_ROOT / "configs/parser_rules/text_ie/v286_strict_adjudication.arc",
    "test": PROJECT_ROOT / "tests/test_spacy_strict_adjudication_v286.py",
}
...
def verify_snapshot(config: dict[str, object]) -> None:
    ...
    expected = dict(config["v286_code_snapshot_sha256"])
    mismatch = {
        name: sha256_file(path)
        for name, path in V286_SNAPSHOT.items()
        if sha256_file(path) != expected[name]
    }
    if mismatch:
        raise ValueError(f"V2.8.6 changed after twenty-ninth declaration: {mismatch}")
...
def documents() -> dict[str, object]:
    ...
    for source in config["sources"]:
        ...
        if sha256_file(path) != source["sha256"]:
            raise ValueError(f"V2.8.6 twenty-ninth source hash mismatch: {ticker}")
...
def _generate(document: object):
    return augment_candidates_v286(document, tuple(parent._generate(document)))
...
def candidate_pool() -> pd.DataFrame:
    ...
    return build_candidate_pool(
        ...
        route_blocks=route_document_blocks_v286,
    )
...
def candidate_blocks() -> pd.DataFrame:
    ...
    return select_candidate_blocks(
        pool,
        config,
        benchmark_label="V2.8.6 twenty-ninth holdout",
    )
