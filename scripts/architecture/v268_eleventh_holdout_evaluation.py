...
def evaluate_eleventh_holdout():
    ...
    summary.loc[:, "version"] = "PLATFORM_V2_6_8_ELEVENTH_DISJOINT_HOLDOUT"
    summary.loc[:, "status"] = (
        "V268_INDEPENDENT_RESEARCH_BENCHMARK_PASS"
        if passed
...
def main() -> int:
    detail, summary, ladder, rejections = evaluate_eleventh_holdout()
    write_csv_artifacts(
        OUTPUT,
        {
            "eleventh_holdout_evaluation": detail,
            "eleventh_holdout_summary": summary,
            "eleventh_holdout_ladder": ladder,
            "eleventh_holdout_rejection_taxonomy": rejections,
        },
    )
    metadata = {
        "version": "PLATFORM_V2_6_8_ELEVENTH_DISJOINT_HOLDOUT",
    ...
    (OUTPUT / "eleventh_holdout_metadata.json").write_text(
