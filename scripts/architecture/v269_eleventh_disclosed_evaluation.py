...
from scripts.architecture.v268_eleventh_holdout_annotations import ANNOTATIONS
from scripts.architecture.v268_eleventh_holdout_candidates import (
    CANDIDATES,
    CONFIG,
    documents,
    load_config,
...
def _development_snapshot(_: dict[str, object]) -> None:
    """V268 inputs remain frozen; V269 is intentionally under development."""
...
def evaluate_eleventh_disclosed():
    with _inputs():
    ...
    summary.loc[:, "version"] = "PLATFORM_V2_6_9_ELEVENTH_DISCLOSED_DEVELOPMENT"
    summary.loc[:, "status"] = (
        "V269_DISCLOSED_DEVELOPMENT_PASS"
        if passed
...
def main() -> int:
    detail, summary, ladder, rejections = evaluate_eleventh_disclosed()
    write_csv_artifacts(
        OUTPUT,
        {
            "eleventh_disclosed_evaluation": detail,
            "eleventh_disclosed_summary": summary,
            "eleventh_disclosed_ladder": ladder,
            "eleventh_disclosed_rejection_taxonomy": rejections,
        },
    )
    metadata = {
        "version": "PLATFORM_V2_6_9_ELEVENTH_DISCLOSED_DEVELOPMENT",
        ...
        "holdout_disclosed_before_v269_changes": True,
        "hmrb_runtime_dependency": False,
        "llm_backend_used": False,
    }
    OUTPUT.mkdir(parents=True, exist_ok=True)
    (OUTPUT / "eleventh_disclosed_metadata.json").write_text(
