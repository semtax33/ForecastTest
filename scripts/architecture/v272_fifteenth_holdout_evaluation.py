...
def evaluate_fifteenth_holdout():
    with _frozen_inputs():
    ...
    summary.loc[:, "version"] = "PLATFORM_V2_7_2_FIFTEENTH_DISJOINT_HOLDOUT"
    summary.loc[:, "status"] = (
        "V272_INDEPENDENT_RESEARCH_BENCHMARK_PASS"
        if passed
...
def main() -> int:
    detail, summary, ladder, rejections = evaluate_fifteenth_holdout()
    write_csv_artifacts(
        OUTPUT,
        {
            "fifteenth_holdout_evaluation": detail,
            "fifteenth_holdout_summary": summary,
            "fifteenth_holdout_ladder": ladder,
            "fifteenth_holdout_rejection_taxonomy": rejections,
        },
    )
    metadata = {
        "version": "PLATFORM_V2_7_2_FIFTEENTH_DISJOINT_HOLDOUT",
    ...
    (OUTPUT / "fifteenth_holdout_metadata.json").write_text(
