...
CONFIG = PROJECT_ROOT / "configs/certification/platform_v278_twenty_first_holdout.toml"
OUTPUT = PROJECT_ROOT / "output/platform_v2_7_8_spacy_semantic_roles"
CANDIDATES = OUTPUT / "twenty_first_holdout_candidates.csv"
...
def verify_snapshot(config: dict[str, object]) -> None:
    ...
    if mismatch:
        raise ValueError(f"V2.7.8 changed after twenty-first holdout declaration: {mismatch}")
...
def documents() -> dict[str, object]:
    ...
    for source in config["sources"]:
        ...
        if sha256_file(path) != source["sha256"]:
            raise ValueError(f"V2.7.8 twenty-first holdout source hash mismatch: {ticker}")
...
def candidate_blocks() -> pd.DataFrame:
    ...
    return select_candidate_blocks(
        pool, config, benchmark_label="V2.7.8 twenty-first holdout"
    )
