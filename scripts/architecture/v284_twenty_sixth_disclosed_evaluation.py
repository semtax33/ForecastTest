...
from scripts.architecture.v283_twenty_sixth_holdout_annotations import ANNOTATIONS
from scripts.architecture.v283_twenty_sixth_holdout_candidates import (
    CANDIDATES,
    CONFIG,
    documents,
    load_config,
...
def evaluate_disclosed_twenty_sixth():
    ...
    summary.loc[:, "version"] = "PLATFORM_V2_8_4_DISCLOSED_TWENTY_SIXTH"
    summary.loc[:, "status"] = "V284_DISCLOSED_DEVELOPMENT_PASS" if passed else "HOLD_RESEARCH_UNFROZEN"
    return detail, summary, ladder, rejections


def main() -> int:
    detail, summary, ladder, rejections = evaluate_disclosed_twenty_sixth()
    write_csv_artifacts(
        OUTPUT,
        {
            "twenty_sixth_disclosed_evaluation": detail,
            "twenty_sixth_disclosed_summary": summary,
            "twenty_sixth_disclosed_ladder": ladder,
            "twenty_sixth_disclosed_rejection_taxonomy": rejections,
        },
    )
    metadata = {
        "version": "PLATFORM_V2_8_4_DISCLOSED_TWENTY_SIXTH",
