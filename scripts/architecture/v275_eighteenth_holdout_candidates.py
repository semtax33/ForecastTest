...
from equity_platform.text_ie.v275.router import route_document_blocks_v275
from equity_platform.text_ie.v275.semantics import augment_candidates_v275
from scripts.architecture.parser_holdout import build_candidate_pool, select_candidate_blocks
from scripts.architecture.v274_seventeenth_holdout_candidates import _generate as generate_v274


CONFIG = PROJECT_ROOT / "configs/certification/platform_v275_eighteenth_holdout.toml"
OUTPUT = PROJECT_ROOT / "output/platform_v2_7_5_spacy_document_context"
CANDIDATES = OUTPUT / "eighteenth_holdout_candidates.csv"
...
def verify_snapshot(config: dict[str, object]) -> None:
    ...
    if mismatch:
        raise ValueError(f"V2.7.5 changed after eighteenth holdout declaration: {mismatch}")
...
def documents() -> dict[str, object]:
    ...
    for source in config["sources"]:
        ...
        if sha256_file(path) != source["sha256"]:
            raise ValueError(f"V2.7.5 eighteenth holdout source hash mismatch: {ticker}")
...
def _generate(document: object):
    return augment_candidates_v275(document, generate_v274(document))
...
def candidate_pool() -> pd.DataFrame:
    ...
    return build_candidate_pool(
        ...
        route_blocks=route_document_blocks_v275,
    )
...
def candidate_blocks() -> pd.DataFrame:
    ...
    return select_candidate_blocks(
        pool, config, benchmark_label="V2.7.5 eighteenth holdout"
    )
