...
from equity_platform.text_ie.v287 import RULE_PATH, extract_text_kpis_v287
import scripts.architecture.v261_fifth_holdout_evaluation as scoring
from scripts.architecture.v286_twenty_ninth_holdout_annotations import ANNOTATIONS
from scripts.architecture.v286_twenty_ninth_holdout_candidates import (
    CANDIDATES,
    CONFIG,
    documents,
    load_config,
...
def _frozen_inputs():
    replacements = {
        ...
        "extract_text_kpis_v261": extract_text_kpis_v287,
    }
...
def evaluate_twenty_ninth_disclosed():
    with _frozen_inputs():
    ...
    summary.loc[:, "version"] = "PLATFORM_V2_8_7_TWENTY_NINTH_DISCLOSED_REMEDIATION"
    summary.loc[:, "status"] = "V287_DISCLOSED_REMEDIATION_PASS" if passed else "HOLD_RESEARCH_UNFROZEN"
    return detail, summary, ladder, rejections


def main() -> int:
    detail, summary, ladder, rejections = evaluate_twenty_ninth_disclosed()
    write_csv_artifacts(OUTPUT, {
        "twenty_ninth_disclosed_evaluation": detail,
        "twenty_ninth_disclosed_summary": summary,
        "twenty_ninth_disclosed_ladder": ladder,
        "twenty_ninth_disclosed_rejection_taxonomy": rejections,
    })
    metadata = {
        "version": "PLATFORM_V2_8_7_TWENTY_NINTH_DISCLOSED_REMEDIATION",
    ...
    (OUTPUT / "twenty_ninth_disclosed_metadata.json").write_text(
