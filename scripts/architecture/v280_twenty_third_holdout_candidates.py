...
from equity_platform.text_ie.v280.router import route_document_blocks_v280
from equity_platform.text_ie.v280.semantics import augment_candidates_v280
...
CONFIG = PROJECT_ROOT / "configs/certification/platform_v280_twenty_third_holdout.toml"
OUTPUT = PROJECT_ROOT / "output/platform_v2_8_0_hierarchical_semantic_context"
CANDIDATES = OUTPUT / "twenty_third_holdout_candidates.csv"
...
SNAPSHOT = {
    ...
    **{
    ...
    "v279_rules": PROJECT_ROOT / "configs/parser_rules/text_ie/v279_semantic_ownership.arc",
    "v280_rules": PROJECT_ROOT / "configs/parser_rules/text_ie/v280_semantic_context.arc",
}
for version in range(271, 281):
...
def verify_snapshot(config: dict[str, object]) -> None:
    ...
    if mismatch:
        raise ValueError(f"V2.8.0 changed after twenty-third declaration: {mismatch}")
...
def documents() -> dict[str, object]:
    ...
    for source in config["sources"]:
        ...
        if sha256_file(path) != source["sha256"]:
            raise ValueError(f"V2.8.0 twenty-third source hash mismatch: {ticker}")
...
def _generate(document: object):
    inherited = augment_candidates_v278(document, tuple(generate_v276(document)))
    return augment_candidates_v280(document, inherited)
...
def candidate_pool() -> pd.DataFrame:
    ...
    return build_candidate_pool(
        ...
        route_blocks=route_document_blocks_v280,
    )
...
def candidate_blocks() -> pd.DataFrame:
    ...
    return select_candidate_blocks(
        pool, config, benchmark_label="V2.8.0 twenty-third holdout"
    )
