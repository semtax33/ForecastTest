...
def evaluate_eighteenth_holdout():
    with _frozen_inputs():
    ...
    summary.loc[:, "version"] = "PLATFORM_V2_7_5_EIGHTEENTH_DISJOINT_HOLDOUT"
    summary.loc[:, "status"] = (
        "V275_INDEPENDENT_RESEARCH_BENCHMARK_PASS"
        if passed
...
def main() -> int:
    detail, summary, ladder, rejections = evaluate_eighteenth_holdout()
    write_csv_artifacts(
        OUTPUT,
        {
            "eighteenth_holdout_evaluation": detail,
            "eighteenth_holdout_summary": summary,
            "eighteenth_holdout_ladder": ladder,
            "eighteenth_holdout_rejection_taxonomy": rejections,
        },
    )
    metadata = {
        "version": "PLATFORM_V2_7_5_EIGHTEENTH_DISJOINT_HOLDOUT",
        ...
        "inherited_context_categories": ["DECLARED_MONEY_SCALE", "GUIDANCE", "TABLE_FAMILY"],
    ...
    (OUTPUT / "eighteenth_holdout_metadata.json").write_text(
