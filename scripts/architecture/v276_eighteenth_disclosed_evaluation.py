...
from equity_platform.text_ie.v276 import RULE_PATH, extract_text_kpis_v276
import scripts.architecture.v261_fifth_holdout_evaluation as scoring
from scripts.architecture.v275_eighteenth_holdout_annotations import ANNOTATIONS
from scripts.architecture.v275_eighteenth_holdout_candidates import (
    CANDIDATES,
    CONFIG,
    documents,
    load_config,
...
def _frozen_inputs():
    replacements = {
        ...
        "extract_text_kpis_v261": extract_text_kpis_v276,
    }
...
def evaluate_eighteenth_disclosed():
    with _frozen_inputs():
    ...
    summary.loc[:, "version"] = "PLATFORM_V2_7_6_EIGHTEENTH_DISCLOSED_DEVELOPMENT"
    summary.loc[:, "status"] = (
        "V276_DISCLOSED_DEVELOPMENT_PASS" if passed else "HOLD_RESEARCH_UNFROZEN"
    )
    return detail, summary, ladder, rejections


def main() -> int:
    detail, summary, ladder, rejections = evaluate_eighteenth_disclosed()
    write_csv_artifacts(
        OUTPUT,
        {
            "eighteenth_disclosed_evaluation": detail,
            "eighteenth_disclosed_summary": summary,
            "eighteenth_disclosed_ladder": ladder,
            "eighteenth_disclosed_rejection_taxonomy": rejections,
        },
    )
    metadata = {
        "version": "PLATFORM_V2_7_6_EIGHTEENTH_DISCLOSED_DEVELOPMENT",
