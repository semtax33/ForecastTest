...
CONFIG = PROJECT_ROOT / "configs/certification/platform_v284_twenty_seventh_holdout.toml"
OUTPUT = PROJECT_ROOT / "output/platform_v2_8_4_economic_roles"
CANDIDATES = OUTPUT / "twenty_seventh_holdout_candidates.csv"
...
def verify_snapshot(config: dict[str, object]) -> None:
    ...
    mismatch = {
        ...
        for name, path in V284_SNAPSHOT.items()
        if sha256_file(path) != expected[name]
    }
    if mismatch:
        raise ValueError(f"V2.8.4 changed after twenty-seventh declaration: {mismatch}")
...
def documents() -> dict[str, object]:
    ...
    for source in config["sources"]:
        ...
        if sha256_file(path) != source["sha256"]:
            raise ValueError(f"V2.8.4 twenty-seventh source hash mismatch: {ticker}")
...
def candidate_pool() -> pd.DataFrame:
    ...
    return build_candidate_pool(
        ...
        route_blocks=route_document_blocks_v284,
    )
...
def candidate_blocks() -> pd.DataFrame:
    ...
    return select_candidate_blocks(pool, config, benchmark_label="V2.8.4 twenty-seventh holdout")
