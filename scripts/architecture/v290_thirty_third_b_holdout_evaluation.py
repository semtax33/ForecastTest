...
EVALUATOR = PROJECT_ROOT / "scripts/architecture/v290_thirty_third_b_holdout_evaluation.py"


def verify_independent_freeze(config: dict[str, object]) -> None:
    verify_snapshot(config)
    selection = dict(config["selection_snapshot"])
    ...
    checks = {
        "candidate_generator": (PROJECT_ROOT / str(selection["generator"]), str(selection["generator_sha256"])),
    ...
    if mismatch:
        raise ValueError(f"V2.9.0 thirty-third-B independent freeze mismatch: {mismatch}")
...
def evaluate_thirty_third_b_holdout():
    with _frozen_inputs():
    ...
    summary.loc[:, "version"] = "PLATFORM_V2_9_0_THIRTY_THIRD_B_DISJOINT_HOLDOUT"
    summary.loc[:, "status"] = (
        "V290_INDEPENDENT_RESEARCH_BENCHMARK_PASS"
        if passed
...
def main() -> int:
    detail, summary, ladder, rejections = evaluate_thirty_third_b_holdout()
    write_csv_artifacts(OUTPUT, {
        "thirty_third_b_holdout_evaluation": detail,
        "thirty_third_b_holdout_summary": summary,
        "thirty_third_b_holdout_ladder": ladder,
        "thirty_third_b_holdout_rejection_taxonomy": rejections,
    })
    metadata = {
        "version": "PLATFORM_V2_9_0_THIRTY_THIRD_B_DISJOINT_HOLDOUT",
    ...
    (OUTPUT / "thirty_third_b_holdout_metadata.json").write_text(
