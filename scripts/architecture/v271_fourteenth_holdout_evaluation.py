...
def evaluate_fourteenth_holdout():
    with _frozen_inputs():
    ...
    summary.loc[:, "version"] = "PLATFORM_V2_7_1_FOURTEENTH_DISJOINT_HOLDOUT"
    summary.loc[:, "status"] = (
        "V271_INDEPENDENT_RESEARCH_BENCHMARK_PASS"
        if passed
...
def main() -> int:
    detail, summary, ladder, rejections = evaluate_fourteenth_holdout()
    write_csv_artifacts(
        OUTPUT,
        {
            "fourteenth_holdout_evaluation": detail,
            "fourteenth_holdout_summary": summary,
            "fourteenth_holdout_ladder": ladder,
            "fourteenth_holdout_rejection_taxonomy": rejections,
        },
    )
    metadata = {
        "version": "PLATFORM_V2_7_1_FOURTEENTH_DISJOINT_HOLDOUT",
    ...
    (OUTPUT / "fourteenth_holdout_metadata.json").write_text(
