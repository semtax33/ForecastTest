...
def evaluate_seventh_holdout():
    ...
    summary.loc[:, "version"] = "PLATFORM_V2_6_4_SEVENTH_DISJOINT_HOLDOUT"
    summary.loc[:, "status"] = (
        "V264_RESEARCH_BENCHMARK_PASS" if passed else "HOLD_RESEARCH_UNFROZEN"
    )
    return detail, summary, ladder, rejections


def main() -> int:
    detail, summary, ladder, rejections = evaluate_seventh_holdout()
    write_csv_artifacts(
        OUTPUT,
        {
            "seventh_holdout_evaluation": detail,
            "seventh_holdout_summary": summary,
            "seventh_holdout_ladder": ladder,
            "seventh_holdout_rejection_taxonomy": rejections,
        },
    )
    metadata = {
        "version": "PLATFORM_V2_6_4_SEVENTH_DISJOINT_HOLDOUT",
    ...
    (OUTPUT / "seventh_holdout_metadata.json").write_text(
