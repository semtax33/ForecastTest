...
from scripts.architecture.v276_nineteenth_holdout_annotations import ANNOTATIONS
...
def evaluate_nineteenth_holdout():
    with _frozen_inputs():
    ...
    summary.loc[:, "version"] = "PLATFORM_V2_7_6_NINETEENTH_DISJOINT_HOLDOUT"
    summary.loc[:, "status"] = (
        "V276_INDEPENDENT_RESEARCH_BENCHMARK_PASS"
        if passed
...
def main() -> int:
    detail, summary, ladder, rejections = evaluate_nineteenth_holdout()
    write_csv_artifacts(
        OUTPUT,
        {
            "nineteenth_holdout_evaluation": detail,
            "nineteenth_holdout_summary": summary,
            "nineteenth_holdout_ladder": ladder,
            "nineteenth_holdout_rejection_taxonomy": rejections,
        },
    )
    metadata = {
        "version": "PLATFORM_V2_7_6_NINETEENTH_DISJOINT_HOLDOUT",
    ...
    (OUTPUT / "nineteenth_holdout_metadata.json").write_text(
