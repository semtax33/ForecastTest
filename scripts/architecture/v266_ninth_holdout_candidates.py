...
from equity_platform.text_ie.v266.router import route_document_blocks_v266
...
CONFIG = PROJECT_ROOT / "configs/certification/platform_v266_ninth_holdout.toml"
OUTPUT = PROJECT_ROOT / "output/platform_v2_6_6_spacy_semantic_firewall"
CANDIDATES = OUTPUT / "ninth_holdout_candidates.csv"
...
SNAPSHOT = {
    ...
    "v266_init": PROJECT_ROOT / "equity_platform/text_ie/v266/__init__.py",
...
def verify_snapshot(config: dict[str, object]) -> None:
    ...
    if mismatch:
        raise ValueError(f"V2.6.6 changed after ninth holdout declaration: {mismatch}")
...
def documents() -> dict[str, object]:
    ...
    for source in config["sources"]:
        ...
        if sha256_file(path) != source["sha256"]:
            raise ValueError(f"V2.6.6 ninth holdout source hash mismatch: {ticker}")
...
def _generate(document: object):
    ...
    return augment_candidates_v266(document, v265)
...
def candidate_pool() -> pd.DataFrame:
    ...
    return build_candidate_pool(
        ...
        route_blocks=route_document_blocks_v266,
    )
...
def candidate_blocks() -> pd.DataFrame:
    ...
    return select_candidate_blocks(
        candidate_pool(),
        config,
        benchmark_label="V2.6.6 ninth holdout",
    )
