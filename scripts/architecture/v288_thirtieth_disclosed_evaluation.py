...
from equity_platform.text_ie.v288 import RULE_PATH, extract_text_kpis_v288
import scripts.architecture.v261_fifth_holdout_evaluation as scoring
from scripts.architecture.v287_thirtieth_holdout_annotations import ANNOTATIONS
from scripts.architecture.v287_thirtieth_holdout_candidates import (
    CANDIDATES,
    CONFIG,
    documents,
    load_config,
...
def _frozen_inputs():
    replacements = {
        ...
        "extract_text_kpis_v261": extract_text_kpis_v288,
    }
...
def evaluate_thirtieth_disclosed():
    with _frozen_inputs():
    ...
    summary.loc[:, "version"] = "PLATFORM_V2_8_8_THIRTIETH_DISCLOSED_REMEDIATION"
    summary.loc[:, "status"] = "V288_DISCLOSED_REMEDIATION_PASS" if passed else "HOLD_RESEARCH_UNFROZEN"
    return detail, summary, ladder, rejections


def main() -> int:
    detail, summary, ladder, rejections = evaluate_thirtieth_disclosed()
    write_csv_artifacts(OUTPUT, {
        "thirtieth_disclosed_evaluation": detail,
        "thirtieth_disclosed_summary": summary,
        "thirtieth_disclosed_ladder": ladder,
        "thirtieth_disclosed_rejection_taxonomy": rejections,
    })
    metadata = {
        "version": "PLATFORM_V2_8_8_THIRTIETH_DISCLOSED_REMEDIATION",
    ...
    (OUTPUT / "thirtieth_disclosed_metadata.json").write_text(
