...
CONFIG = PROJECT_ROOT / "configs/certification/platform_v262_sixth_holdout.toml"
OUTPUT = PROJECT_ROOT / "output/platform_v2_6_2_range_and_route_guards"
CANDIDATES = OUTPUT / "sixth_holdout_candidates.csv"
ARCANA_IR = Path("D:/Programming/python_example/Arcana/data-lake/bronze/sec/fillings/ir")
SNAPSHOT = {
    "router": PROJECT_ROOT / "equity_platform/text_ie/v262/router.py",
    "semantics": PROJECT_ROOT / "equity_platform/text_ie/v262/semantics.py",
    "runtime": PROJECT_ROOT / "equity_platform/text_ie/v262/runtime.py",
    "init": PROJECT_ROOT / "equity_platform/text_ie/v262/__init__.py",
...
def verify_snapshot(config: dict[str, object]) -> None:
    ...
    if mismatch:
        raise ValueError(f"V2.6.2 changed after sixth holdout declaration: {mismatch}")
...
def documents() -> dict[str, object]:
    ...
    for source in config["sources"]:
        ...
        if sha256_file(path) != source["sha256"]:
            raise ValueError(f"V2.6.2 sixth holdout source hash mismatch: {ticker}")
...
def candidate_pool() -> pd.DataFrame:
    ...
    for ticker, document in documents().items():
        ...
        route_by_span = {
            ...
            for route in route_document_blocks_v262(document)
...
def candidate_blocks() -> pd.DataFrame:
    ...
    for ticker in config["evaluation_tickers"]:
        ...
        remaining = subset.loc[~subset["candidate_id"].isin(chosen_ids)].sort_values(
            "sampling_key"
        )
        chosen_ids.update(
            str(value)
            for value in remaining.head(max(0, target - len(chosen_ids)))["candidate_id"]
        )
    ...
    if len(selected) < expected:
        supplement = pool.loc[~pool["candidate_id"].isin(selected_ids)].sort_values(
            "sampling_key"
        )
        selected_ids.update(
            str(value)
            for value in supplement.head(expected - len(selected))["candidate_id"]
        )
        selected = pool.loc[pool["candidate_id"].isin(selected_ids)].copy()
    if len(selected) != expected:
        raise ValueError(
            f"V2.6.2 sixth holdout requires {expected} blocks, selected {len(selected)}"
        )
