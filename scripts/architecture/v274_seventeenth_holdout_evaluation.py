...
def evaluate_seventeenth_holdout():
    with _frozen_inputs():
    ...
    summary.loc[:, "version"] = "PLATFORM_V2_7_4_SEVENTEENTH_DISJOINT_HOLDOUT"
    summary.loc[:, "status"] = (
        "V274_INDEPENDENT_RESEARCH_BENCHMARK_PASS"
        if passed
...
def main() -> int:
    detail, summary, ladder, rejections = evaluate_seventeenth_holdout()
    write_csv_artifacts(
        OUTPUT,
        {
            "seventeenth_holdout_evaluation": detail,
            "seventeenth_holdout_summary": summary,
            "seventeenth_holdout_ladder": ladder,
            "seventeenth_holdout_rejection_taxonomy": rejections,
        },
    )
    metadata = {
        "version": "PLATFORM_V2_7_4_SEVENTEENTH_DISJOINT_HOLDOUT",
    ...
    (OUTPUT / "seventeenth_holdout_metadata.json").write_text(
