...
from equity_platform.text_ie.v268.router import route_document_blocks_v268
from equity_platform.text_ie.v268.semantics import augment_candidates_v268
from scripts.architecture.parser_holdout import (
    build_candidate_pool,
    select_candidate_blocks,
)


CONFIG = PROJECT_ROOT / "configs/certification/platform_v268_eleventh_holdout.toml"
OUTPUT = PROJECT_ROOT / "output/platform_v2_6_8_spacy_semantic_ownership"
CANDIDATES = OUTPUT / "eleventh_holdout_candidates.csv"
...
def verify_snapshot(config: dict[str, object]) -> None:
    ...
    if mismatch:
        raise ValueError(f"V2.6.8 changed after eleventh holdout declaration: {mismatch}")
...
def documents() -> dict[str, object]:
    ...
    for source in config["sources"]:
        ...
        if sha256_file(path) != source["sha256"]:
            raise ValueError(f"V2.6.8 eleventh holdout source hash mismatch: {ticker}")
...
def _generate(document: object):
    ...
    return augment_candidates_v268(document, v267)
...
def candidate_pool() -> pd.DataFrame:
    ...
    return build_candidate_pool(
        ...
        route_blocks=route_document_blocks_v268,
    )
...
def candidate_blocks() -> pd.DataFrame:
    ...
    return select_candidate_blocks(
        candidate_pool(), config, benchmark_label="V2.6.8 eleventh holdout"
    )
