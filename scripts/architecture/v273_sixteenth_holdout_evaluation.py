...
def evaluate_sixteenth_holdout():
    with _frozen_inputs():
    ...
    summary.loc[:, "version"] = "PLATFORM_V2_7_3_SIXTEENTH_DISJOINT_HOLDOUT"
    summary.loc[:, "status"] = (
        "V273_INDEPENDENT_RESEARCH_BENCHMARK_PASS"
        if passed
...
def main() -> int:
    detail, summary, ladder, rejections = evaluate_sixteenth_holdout()
    write_csv_artifacts(
        OUTPUT,
        {
            "sixteenth_holdout_evaluation": detail,
            "sixteenth_holdout_summary": summary,
            "sixteenth_holdout_ladder": ladder,
            "sixteenth_holdout_rejection_taxonomy": rejections,
        },
    )
    metadata = {
        "version": "PLATFORM_V2_7_3_SIXTEENTH_DISJOINT_HOLDOUT",
    ...
    (OUTPUT / "sixteenth_holdout_metadata.json").write_text(
