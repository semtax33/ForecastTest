...
from equity_platform.text_ie.v283 import RULE_PATH, extract_text_kpis_v283
import scripts.architecture.v261_fifth_holdout_evaluation as scoring
from scripts.architecture.v282_twenty_fifth_c_holdout_annotations import ANNOTATIONS
from scripts.architecture.v282_twenty_fifth_c_holdout_candidates import (
    CANDIDATES,
    CONFIG,
    documents,
    load_config,
...
def _development_inputs():
    replacements = {
        ...
        "extract_text_kpis_v261": extract_text_kpis_v283,
    }
...
def evaluate_disclosed_twenty_fifth_c():
    ...
    summary.loc[:, "version"] = "PLATFORM_V2_8_3_DISCLOSED_TWENTY_FIFTH_C"
    summary.loc[:, "status"] = (
        "V283_DISCLOSED_DEVELOPMENT_PASS" if passed else "HOLD_RESEARCH_UNFROZEN"
    )
    return detail, summary, ladder, rejections


def main() -> int:
    detail, summary, ladder, rejections = evaluate_disclosed_twenty_fifth_c()
    write_csv_artifacts(
        OUTPUT,
        {
            "twenty_fifth_c_disclosed_evaluation": detail,
            "twenty_fifth_c_disclosed_summary": summary,
            "twenty_fifth_c_disclosed_ladder": ladder,
            "twenty_fifth_c_disclosed_rejection_taxonomy": rejections,
        },
    )
    metadata = {
        "version": "PLATFORM_V2_8_3_DISCLOSED_TWENTY_FIFTH_C",
