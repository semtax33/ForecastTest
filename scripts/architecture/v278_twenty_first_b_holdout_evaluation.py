...
def evaluate_twenty_first_b_holdout():
    with _frozen_inputs():
    ...
    summary.loc[:, "version"] = "PLATFORM_V2_7_8_TWENTY_FIRST_B_DISJOINT_HOLDOUT"
    summary.loc[:, "status"] = (
        "V278_INDEPENDENT_RESEARCH_BENCHMARK_PASS"
        if passed
...
def main() -> int:
    detail, summary, ladder, rejections = evaluate_twenty_first_b_holdout()
    write_csv_artifacts(
        OUTPUT,
        {
            "twenty_first_b_holdout_evaluation": detail,
            "twenty_first_b_holdout_summary": summary,
            "twenty_first_b_holdout_ladder": ladder,
            "twenty_first_b_holdout_rejection_taxonomy": rejections,
        },
    )
    metadata = {
        "version": "PLATFORM_V2_7_8_TWENTY_FIRST_B_DISJOINT_HOLDOUT",
    ...
    (OUTPUT / "twenty_first_b_holdout_metadata.json").write_text(
