...
CONFIG = PROJECT_ROOT / "configs/certification/platform_v282_twenty_fifth_holdout.toml"
OUTPUT = PROJECT_ROOT / "output/platform_v2_8_2_upper_context"
CANDIDATES = OUTPUT / "twenty_fifth_holdout_candidates.csv"
...
def verify_snapshot(config: dict[str, object]) -> None:
    ...
    mismatch = {
        ...
        for name, path in V282_SNAPSHOT.items()
        if sha256_file(path) != expected[name]
    }
    if mismatch:
        raise ValueError(f"V2.8.2 changed after twenty-fifth declaration: {mismatch}")
...
def documents() -> dict[str, object]:
    ...
    for source in config["sources"]:
        ...
        if sha256_file(path) != source["sha256"]:
            raise ValueError(f"V2.8.2 twenty-fifth source hash mismatch: {ticker}")
...
def candidate_blocks() -> pd.DataFrame:
    ...
    return select_candidate_blocks(pool, config, benchmark_label="V2.8.2 twenty-fifth holdout")
