...
def evaluate_ninth_holdout():
    ...
    summary.loc[:, "version"] = "PLATFORM_V2_6_6_NINTH_DISJOINT_HOLDOUT"
    summary.loc[:, "status"] = (
        "V266_RESEARCH_BENCHMARK_PASS" if passed else "HOLD_RESEARCH_UNFROZEN"
    )
    return detail, summary, ladder, rejections


def main() -> int:
    detail, summary, ladder, rejections = evaluate_ninth_holdout()
    write_csv_artifacts(
        OUTPUT,
        {
            "ninth_holdout_evaluation": detail,
            "ninth_holdout_summary": summary,
            "ninth_holdout_ladder": ladder,
            "ninth_holdout_rejection_taxonomy": rejections,
        },
    )
    metadata = {
        "version": "PLATFORM_V2_6_6_NINTH_DISJOINT_HOLDOUT",
    ...
    (OUTPUT / "ninth_holdout_metadata.json").write_text(
