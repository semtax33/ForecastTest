...
from equity_platform.text_ie.v279 import RULE_PATH, extract_text_kpis_v279
import scripts.architecture.v261_fifth_holdout_evaluation as scoring
from scripts.architecture.v279_twenty_second_holdout_annotations import ANNOTATIONS
from scripts.architecture.v279_twenty_second_holdout_candidates import (
    CANDIDATES,
    CONFIG,
    OUTPUT,
    documents,
...
def _frozen_inputs():
    replacements = {
        ...
        "extract_text_kpis_v261": extract_text_kpis_v279,
    }
...
def evaluate_twenty_second_holdout():
    with _frozen_inputs():
    ...
    summary.loc[:, "version"] = "PLATFORM_V2_7_9_TWENTY_SECOND_DISJOINT_HOLDOUT"
    summary.loc[:, "status"] = (
        "V279_INDEPENDENT_RESEARCH_BENCHMARK_PASS"
        if passed
...
def main() -> int:
    detail, summary, ladder, rejections = evaluate_twenty_second_holdout()
    write_csv_artifacts(
        OUTPUT,
        {
            "twenty_second_holdout_evaluation": detail,
            "twenty_second_holdout_summary": summary,
            "twenty_second_holdout_ladder": ladder,
            "twenty_second_holdout_rejection_taxonomy": rejections,
        },
    )
    metadata = {
        "version": "PLATFORM_V2_7_9_TWENTY_SECOND_DISJOINT_HOLDOUT",
        ...
        "semantic_duplicate_identity_includes_polarity_and_range": True,
    }
    (OUTPUT / "twenty_second_holdout_metadata.json").write_text(
