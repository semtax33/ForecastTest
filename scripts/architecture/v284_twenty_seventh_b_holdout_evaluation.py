...
from equity_platform.text_ie.v284 import RULE_PATH, extract_text_kpis_v284
...
from scripts.architecture.v284_twenty_seventh_b_holdout_candidates import CANDIDATES, CONFIG, OUTPUT, documents, load_config, verify_snapshot
...
def _frozen_inputs():
    replacements = {
        ...
        "extract_text_kpis_v261": extract_text_kpis_v284,
    }
...
def evaluate_twenty_seventh_b_holdout():
    with _frozen_inputs():
    ...
    summary.loc[:, "version"] = "PLATFORM_V2_8_4_TWENTY_SEVENTH_B_DISJOINT_HOLDOUT"
    summary.loc[:, "status"] = "V284_INDEPENDENT_RESEARCH_BENCHMARK_PASS" if passed else "HOLD_RESEARCH_UNFROZEN"
    return detail, summary, ladder, rejections


def main() -> int:
    detail, summary, ladder, rejections = evaluate_twenty_seventh_b_holdout()
    write_csv_artifacts(OUTPUT, {
        "twenty_seventh_b_holdout_evaluation": detail,
        "twenty_seventh_b_holdout_summary": summary,
        "twenty_seventh_b_holdout_ladder": ladder,
        "twenty_seventh_b_holdout_rejection_taxonomy": rejections,
    })
    metadata = {
        "version": "PLATFORM_V2_8_4_TWENTY_SEVENTH_B_DISJOINT_HOLDOUT",
    ...
    (OUTPUT / "twenty_seventh_b_holdout_metadata.json").write_text(json.dumps(metadata, indent=2) + "\n", encoding="utf-8")
