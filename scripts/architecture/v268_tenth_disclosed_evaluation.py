...
from equity_platform.text_ie.v268 import RULE_PATH, extract_text_kpis_v268
...
def _development_snapshot(_: dict[str, object]) -> None:
    """The V267 artifacts stay frozen; V268 source is intentionally under development."""
...
def _inputs():
    replacements = {
        ...
        "extract_text_kpis_v261": extract_text_kpis_v268,
    }
...
def evaluate_tenth_disclosed():
    ...
    summary.loc[:, "version"] = "PLATFORM_V2_6_8_TENTH_DISCLOSED_DEVELOPMENT"
    summary.loc[:, "status"] = (
        "V268_DISCLOSED_DEVELOPMENT_PASS"
        if passed
...
def main() -> int:
    detail, summary, ladder, rejections = evaluate_tenth_disclosed()
    write_csv_artifacts(
        OUTPUT,
        {
            "tenth_disclosed_evaluation": detail,
            "tenth_disclosed_summary": summary,
            "tenth_disclosed_ladder": ladder,
            "tenth_disclosed_rejection_taxonomy": rejections,
        },
    )
    metadata = {
        "version": "PLATFORM_V2_6_8_TENTH_DISCLOSED_DEVELOPMENT",
        ...
        "holdout_disclosed_before_v268_changes": True,
        "hmrb_runtime_dependency": False,
        "llm_backend_used": False,
    }
    OUTPUT.mkdir(parents=True, exist_ok=True)
    (OUTPUT / "tenth_disclosed_metadata.json").write_text(
