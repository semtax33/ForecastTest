...
from equity_platform.text_ie.v267.router import route_document_blocks_v267
...
CONFIG = PROJECT_ROOT / "configs/certification/platform_v267_tenth_holdout.toml"
OUTPUT = PROJECT_ROOT / "output/platform_v2_6_7_spacy_semantic_firewall"
CANDIDATES = OUTPUT / "tenth_holdout_candidates.csv"
...
SNAPSHOT = {
    ...
    "v266_semantics": PROJECT_ROOT / "equity_platform/text_ie/v266/semantics.py",
    "v266_runtime": PROJECT_ROOT / "equity_platform/text_ie/v266/runtime.py",
    "v266_router": PROJECT_ROOT / "equity_platform/text_ie/v266/router.py",
    "v266_rules": PROJECT_ROOT / "configs/parser_rules/text_ie/v266_semantic_laws.arc",
...
def verify_snapshot(config: dict[str, object]) -> None:
    ...
    if mismatch:
        raise ValueError(f"V2.6.7 changed after tenth holdout declaration: {mismatch}")
...
def documents() -> dict[str, object]:
    ...
    for source in config["sources"]:
        ...
        if sha256_file(path) != source["sha256"]:
            raise ValueError(f"V2.6.7 tenth holdout source hash mismatch: {ticker}")
...
def _generate(document: object):
    ...
    return augment_candidates_v267(document, v266)
...
def candidate_pool() -> pd.DataFrame:
    ...
    return build_candidate_pool(
        ...
        route_blocks=route_document_blocks_v267,
    )
...
def candidate_blocks() -> pd.DataFrame:
    ...
    return select_candidate_blocks(
        candidate_pool(), config, benchmark_label="V2.6.7 tenth holdout"
    )
