...
from scripts.architecture.v280_twenty_third_b_holdout_annotations import ANNOTATIONS
...
def evaluate_twenty_third_b_holdout():
    with _frozen_inputs():
    ...
    summary.loc[:, "version"] = "PLATFORM_V2_8_0_TWENTY_THIRD_B_DISJOINT_HOLDOUT"
    summary.loc[:, "status"] = (
        "V280_INDEPENDENT_RESEARCH_BENCHMARK_PASS"
        if passed
...
def main() -> int:
    detail, summary, ladder, rejections = evaluate_twenty_third_b_holdout()
    write_csv_artifacts(
        OUTPUT,
        {
            "twenty_third_b_holdout_evaluation": detail,
            "twenty_third_b_holdout_summary": summary,
            "twenty_third_b_holdout_ladder": ladder,
            "twenty_third_b_holdout_rejection_taxonomy": rejections,
        },
    )
    metadata = {
        "version": "PLATFORM_V2_8_0_TWENTY_THIRD_B_DISJOINT_HOLDOUT",
    ...
    (OUTPUT / "twenty_third_b_holdout_metadata.json").write_text(
