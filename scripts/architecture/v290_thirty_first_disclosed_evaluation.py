...
def evaluate_thirty_first_regression():
    with _frozen_inputs():
    ...
    summary.loc[:, "version"] = "PLATFORM_V2_9_0_THIRTY_FIRST_REGRESSION"
    summary.loc[:, "status"] = "V290_PRIOR_DISCLOSED_REGRESSION_PASS" if passed else "REGRESSION_FAILURE"
    return detail, summary, ladder, rejections


def main() -> int:
    detail, summary, ladder, rejections = evaluate_thirty_first_regression()
    write_csv_artifacts(
        OUTPUT,
        {
            "thirty_first_regression_evaluation": detail,
            "thirty_first_regression_summary": summary,
            "thirty_first_regression_ladder": ladder,
            "thirty_first_regression_rejection_taxonomy": rejections,
        },
    )
    metadata = {
        "version": "PLATFORM_V2_9_0_THIRTY_FIRST_REGRESSION",
        ...
        "prior_disclosed_regression_set": True,
    ...
    (OUTPUT / "thirty_first_regression_metadata.json").write_text(
