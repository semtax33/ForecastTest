...
from scripts.architecture.v288_thirty_first_holdout_annotations import ANNOTATIONS
from scripts.architecture.v288_thirty_first_holdout_candidates import (
    CANDIDATES,
    CONFIG,
    documents,
    load_config,
...
def evaluate_thirty_first_disclosed():
    with _frozen_inputs():
    ...
    summary.loc[:, "version"] = "PLATFORM_V2_8_9_THIRTY_FIRST_DISCLOSED_REMEDIATION"
    summary.loc[:, "status"] = "V289_DISCLOSED_REMEDIATION_PASS" if passed else "HOLD_RESEARCH_UNFROZEN"
    return detail, summary, ladder, rejections


def main() -> int:
    detail, summary, ladder, rejections = evaluate_thirty_first_disclosed()
    write_csv_artifacts(OUTPUT, {
        "thirty_first_disclosed_evaluation": detail,
        "thirty_first_disclosed_summary": summary,
        "thirty_first_disclosed_ladder": ladder,
        "thirty_first_disclosed_rejection_taxonomy": rejections,
    })
    metadata = {
        "version": "PLATFORM_V2_8_9_THIRTY_FIRST_DISCLOSED_REMEDIATION",
    ...
    (OUTPUT / "thirty_first_disclosed_metadata.json").write_text(
