...
def evaluate_thirty_first_holdout():
    with _frozen_inputs():
    ...
    summary.loc[:, "version"] = "PLATFORM_V2_8_8_THIRTY_FIRST_DISJOINT_HOLDOUT"
    summary.loc[:, "status"] = (
        "V288_INDEPENDENT_RESEARCH_BENCHMARK_PASS"
        if passed
...
def main() -> int:
    detail, summary, ladder, rejections = evaluate_thirty_first_holdout()
    write_csv_artifacts(OUTPUT, {
        "thirty_first_holdout_evaluation": detail,
        "thirty_first_holdout_summary": summary,
        "thirty_first_holdout_ladder": ladder,
        "thirty_first_holdout_rejection_taxonomy": rejections,
    })
    metadata = {
        "version": "PLATFORM_V2_8_8_THIRTY_FIRST_DISJOINT_HOLDOUT",
    ...
    (OUTPUT / "thirty_first_holdout_metadata.json").write_text(
