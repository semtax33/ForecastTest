...
from equity_platform.text_ie.v283.router import route_document_blocks_v283
from equity_platform.text_ie.v283.semantics import augment_candidates_v283
from scripts.architecture.parser_holdout import build_candidate_pool, select_candidate_blocks
import scripts.architecture.v282_twenty_fifth_c_holdout_candidates as parent


CONFIG = PROJECT_ROOT / "configs/certification/platform_v283_twenty_sixth_holdout.toml"
OUTPUT = PROJECT_ROOT / "output/platform_v2_8_3_bounded_upper_context"
CANDIDATES = OUTPUT / "twenty_sixth_holdout_candidates.csv"
ARCANA_IR = Path("D:/Programming/python_example/Arcana/data-lake/bronze/sec/fillings/ir")
V283_SNAPSHOT = {
    "semantics": PROJECT_ROOT / "equity_platform/text_ie/v283/semantics.py",
    "runtime": PROJECT_ROOT / "equity_platform/text_ie/v283/runtime.py",
    "router": PROJECT_ROOT / "equity_platform/text_ie/v283/router.py",
    "init": PROJECT_ROOT / "equity_platform/text_ie/v283/__init__.py",
    "rules": PROJECT_ROOT / "configs/parser_rules/text_ie/v283_bounded_upper_context.arc",
}
...
def verify_snapshot(config: dict[str, object]) -> None:
    ...
    expected = dict(config["v283_code_snapshot_sha256"])
    mismatch = {
        name: sha256_file(path)
        for name, path in V283_SNAPSHOT.items()
        if sha256_file(path) != expected[name]
    }
    if mismatch:
        raise ValueError(f"V2.8.3 changed after twenty-sixth declaration: {mismatch}")
...
def documents() -> dict[str, object]:
    ...
    for source in config["sources"]:
        ...
        if sha256_file(path) != source["sha256"]:
            raise ValueError(f"V2.8.3 twenty-sixth source hash mismatch: {ticker}")
...
def _generate(document: object):
    return augment_candidates_v283(document, tuple(parent._generate(document)))
...
def candidate_pool() -> pd.DataFrame:
    ...
    return build_candidate_pool(
        ...
        route_blocks=route_document_blocks_v283,
    )
...
def candidate_blocks() -> pd.DataFrame:
    ...
    if insufficient:
        raise ValueError(
            f"Insufficient eligible blocks before artifact write: {insufficient}"
        )
    return select_candidate_blocks(
        pool,
        config,
        benchmark_label="V2.8.3 twenty-sixth holdout",
    )
