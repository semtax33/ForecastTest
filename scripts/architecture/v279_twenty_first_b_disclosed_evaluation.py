...
from scripts.architecture.v278_twenty_first_b_holdout_annotations import ANNOTATIONS
from scripts.architecture.v278_twenty_first_b_holdout_candidates import (
    CANDIDATES,
    CONFIG,
    documents,
    load_config,
...
def evaluate_disclosed_twenty_first_b():
    ...
    summary.loc[:, "version"] = "PLATFORM_V2_7_9_DISCLOSED_TWENTY_FIRST_B"
    summary.loc[:, "status"] = (
        "V279_DISCLOSED_DEVELOPMENT_PASS" if passed else "HOLD_RESEARCH_UNFROZEN"
    )
    return detail, summary, ladder, rejections


def main() -> int:
    detail, summary, ladder, rejections = evaluate_disclosed_twenty_first_b()
    write_csv_artifacts(
        OUTPUT,
        {
            "twenty_first_b_disclosed_evaluation": detail,
            "twenty_first_b_disclosed_summary": summary,
            "twenty_first_b_disclosed_ladder": ladder,
            "twenty_first_b_disclosed_rejection_taxonomy": rejections,
        },
    )
    metadata = {
        "version": "PLATFORM_V2_7_9_DISCLOSED_TWENTY_FIRST_B",
    ...
    (OUTPUT / "twenty_first_b_disclosed_metadata.json").write_text(
