...
from equity_platform.text_ie.v270.router import route_document_blocks_v270
...
CONFIG = PROJECT_ROOT / "configs/certification/platform_v270_thirteenth_holdout.toml"
OUTPUT = PROJECT_ROOT / "output/platform_v2_7_0_spacy_semantic_ownership"
CANDIDATES = OUTPUT / "thirteenth_holdout_candidates.csv"
...
def verify_snapshot(config: dict[str, object]) -> None:
    ...
    if mismatch:
        raise ValueError(f"V2.7.0 changed after thirteenth holdout declaration: {mismatch}")
...
def documents() -> dict[str, object]:
    ...
    for source in config["sources"]:
        ...
        if sha256_file(path) != source["sha256"]:
            raise ValueError(f"V2.7.0 thirteenth holdout source hash mismatch: {ticker}")
...
def _generate(document: object):
    ...
    return augment_candidates_v270(document, v269)
...
def candidate_pool() -> pd.DataFrame:
    ...
    return build_candidate_pool(
        ...
        route_blocks=route_document_blocks_v270,
    )
...
def candidate_blocks() -> pd.DataFrame:
    ...
    return select_candidate_blocks(
        candidate_pool(), config, benchmark_label="V2.7.0 thirteenth holdout"
    )
