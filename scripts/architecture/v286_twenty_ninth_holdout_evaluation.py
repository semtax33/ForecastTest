...
def evaluate_twenty_ninth_holdout():
    with _frozen_inputs():
    ...
    summary.loc[:, "version"] = "PLATFORM_V2_8_6_TWENTY_NINTH_DISJOINT_HOLDOUT"
    summary.loc[:, "status"] = (
        "V286_INDEPENDENT_RESEARCH_BENCHMARK_PASS"
        if passed
...
def main() -> int:
    detail, summary, ladder, rejections = evaluate_twenty_ninth_holdout()
    write_csv_artifacts(OUTPUT, {
        "twenty_ninth_holdout_evaluation": detail,
        "twenty_ninth_holdout_summary": summary,
        "twenty_ninth_holdout_ladder": ladder,
        "twenty_ninth_holdout_rejection_taxonomy": rejections,
    })
    metadata = {
        "version": "PLATFORM_V2_8_6_TWENTY_NINTH_DISJOINT_HOLDOUT",
    ...
    (OUTPUT / "twenty_ninth_holdout_metadata.json").write_text(
