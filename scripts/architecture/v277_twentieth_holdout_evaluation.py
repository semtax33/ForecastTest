...
def evaluate_twentieth_holdout():
    with _frozen_inputs():
    ...
    summary.loc[:, "version"] = "PLATFORM_V2_7_7_TWENTIETH_B_DISJOINT_HOLDOUT"
    summary.loc[:, "status"] = (
        "V277_INDEPENDENT_RESEARCH_BENCHMARK_PASS"
        if passed
...
def main() -> int:
    detail, summary, ladder, rejections = evaluate_twentieth_holdout()
    write_csv_artifacts(
        OUTPUT,
        {
            "twentieth_holdout_evaluation": detail,
            "twentieth_holdout_summary": summary,
            "twentieth_holdout_ladder": ladder,
            "twentieth_holdout_rejection_taxonomy": rejections,
        },
    )
    metadata = {
        "version": "PLATFORM_V2_7_7_TWENTIETH_B_DISJOINT_HOLDOUT",
    ...
    (OUTPUT / "twentieth_holdout_metadata.json").write_text(
