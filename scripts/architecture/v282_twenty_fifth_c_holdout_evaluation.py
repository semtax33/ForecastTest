...
def evaluate_twenty_fifth_c_holdout():
    with _frozen_inputs():
    ...
    summary.loc[:, "version"] = "PLATFORM_V2_8_2_TWENTY_FIFTH_C_DISJOINT_HOLDOUT"
    summary.loc[:, "status"] = (
        "V282_INDEPENDENT_RESEARCH_BENCHMARK_PASS"
        if passed
...
def main() -> int:
    detail, summary, ladder, rejections = evaluate_twenty_fifth_c_holdout()
    write_csv_artifacts(
        OUTPUT,
        {
            "twenty_fifth_c_holdout_evaluation": detail,
            "twenty_fifth_c_holdout_summary": summary,
            "twenty_fifth_c_holdout_ladder": ladder,
            "twenty_fifth_c_holdout_rejection_taxonomy": rejections,
        },
    )
    metadata = {
        "version": "PLATFORM_V2_8_2_TWENTY_FIFTH_C_DISJOINT_HOLDOUT",
    ...
    (OUTPUT / "twenty_fifth_c_holdout_metadata.json").write_text(
