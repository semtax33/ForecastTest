...
from equity_platform.text_ie.v269.router import route_document_blocks_v269
...
CONFIG = PROJECT_ROOT / "configs/certification/platform_v269_twelfth_holdout.toml"
OUTPUT = PROJECT_ROOT / "output/platform_v2_6_9_spacy_semantic_ownership"
CANDIDATES = OUTPUT / "twelfth_holdout_candidates.csv"
...
SNAPSHOT = {
    ...
    "v267_semantics": PROJECT_ROOT / "equity_platform/text_ie/v267/semantics.py",
    "v267_runtime": PROJECT_ROOT / "equity_platform/text_ie/v267/runtime.py",
    "v267_router": PROJECT_ROOT / "equity_platform/text_ie/v267/router.py",
    "v267_init": PROJECT_ROOT / "equity_platform/text_ie/v267/__init__.py",
    "v267_rules": PROJECT_ROOT / "configs/parser_rules/text_ie/v267_semantic_laws.arc",
    "v268_semantics": PROJECT_ROOT / "equity_platform/text_ie/v268/semantics.py",
    "v268_runtime": PROJECT_ROOT / "equity_platform/text_ie/v268/runtime.py",
    "v268_router": PROJECT_ROOT / "equity_platform/text_ie/v268/router.py",
    "v268_init": PROJECT_ROOT / "equity_platform/text_ie/v268/__init__.py",
    "v268_rules": PROJECT_ROOT / "configs/parser_rules/text_ie/v268_semantic_laws.arc",
    "v269_semantics": PROJECT_ROOT / "equity_platform/text_ie/v269/semantics.py",
    "v269_runtime": PROJECT_ROOT / "equity_platform/text_ie/v269/runtime.py",
    "v269_router": PROJECT_ROOT / "equity_platform/text_ie/v269/router.py",
    "v269_init": PROJECT_ROOT / "equity_platform/text_ie/v269/__init__.py",
    "v269_rules": PROJECT_ROOT / "configs/parser_rules/text_ie/v269_semantic_laws.arc",
}
...
def verify_snapshot(config: dict[str, object]) -> None:
    ...
    if mismatch:
        raise ValueError(f"V2.6.9 changed after twelfth holdout declaration: {mismatch}")
...
def documents() -> dict[str, object]:
    ...
    for source in config["sources"]:
        ...
        if sha256_file(path) != source["sha256"]:
            raise ValueError(f"V2.6.9 twelfth holdout source hash mismatch: {ticker}")
...
def _generate(document: object):
    ...
    return augment_candidates_v269(document, v268)
...
def candidate_pool() -> pd.DataFrame:
    ...
    return build_candidate_pool(
        ...
        route_blocks=route_document_blocks_v269,
    )
...
def candidate_blocks() -> pd.DataFrame:
    ...
    return select_candidate_blocks(
        candidate_pool(), config, benchmark_label="V2.6.9 twelfth holdout"
    )
