...
from equity_platform.text_ie.v280 import RULE_PATH, extract_text_kpis_v280
...
def _development_inputs():
    replacements = {
        ...
        "extract_text_kpis_v261": extract_text_kpis_v280,
    }
...
def evaluate_disclosed_twenty_second():
    ...
    summary.loc[:, "version"] = "PLATFORM_V2_8_0_DISCLOSED_TWENTY_SECOND"
    summary.loc[:, "status"] = (
        "V280_DISCLOSED_DEVELOPMENT_PASS" if passed else "HOLD_RESEARCH_UNFROZEN"
    )
    return detail, summary, ladder, rejections


def main() -> int:
    detail, summary, ladder, rejections = evaluate_disclosed_twenty_second()
    write_csv_artifacts(
        OUTPUT,
        {
            "twenty_second_disclosed_evaluation": detail,
            "twenty_second_disclosed_summary": summary,
            "twenty_second_disclosed_ladder": ladder,
            "twenty_second_disclosed_rejection_taxonomy": rejections,
        },
    )
    metadata = {
        "version": "PLATFORM_V2_8_0_DISCLOSED_TWENTY_SECOND",
    ...
    (OUTPUT / "twenty_second_disclosed_metadata.json").write_text(
