...
from equity_platform.text_ie.v272.router import route_document_blocks_v272
from equity_platform.text_ie.v272.semantics import augment_candidates_v272
from scripts.architecture.parser_holdout import build_candidate_pool, select_candidate_blocks
from scripts.architecture.v271_fourteenth_holdout_candidates import _generate as generate_v271


CONFIG = PROJECT_ROOT / "configs/certification/platform_v272_fifteenth_holdout.toml"
OUTPUT = PROJECT_ROOT / "output/platform_v2_7_2_spacy_semantic_ownership"
CANDIDATES = OUTPUT / "fifteenth_holdout_candidates.csv"
...
def verify_snapshot(config: dict[str, object]) -> None:
    ...
    if mismatch:
        raise ValueError(f"V2.7.2 changed after fifteenth holdout declaration: {mismatch}")
...
def documents() -> dict[str, object]:
    ...
    for source in config["sources"]:
        ...
        if sha256_file(path) != source["sha256"]:
            raise ValueError(f"V2.7.2 fifteenth holdout source hash mismatch: {ticker}")
...
def _generate(document: object):
    return augment_candidates_v272(document, generate_v271(document))
...
def candidate_pool() -> pd.DataFrame:
    ...
    return build_candidate_pool(
        ...
        route_blocks=route_document_blocks_v272,
    )
...
def candidate_blocks() -> pd.DataFrame:
    ...
    return select_candidate_blocks(
        candidate_pool(), config, benchmark_label="V2.7.2 fifteenth holdout"
    )
