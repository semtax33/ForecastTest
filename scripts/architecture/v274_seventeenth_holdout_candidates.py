...
from equity_platform.text_ie.v274.router import route_document_blocks_v274
from equity_platform.text_ie.v274.semantics import augment_candidates_v274
from scripts.architecture.parser_holdout import build_candidate_pool, select_candidate_blocks
from scripts.architecture.v273_sixteenth_holdout_candidates import _generate as generate_v273


CONFIG = PROJECT_ROOT / "configs/certification/platform_v274_seventeenth_holdout.toml"
OUTPUT = PROJECT_ROOT / "output/platform_v2_7_4_spacy_bounded_context"
CANDIDATES = OUTPUT / "seventeenth_holdout_candidates.csv"
...
# Keep the manifest's stable key for package initializers.
for version in range(271, 275):
...
def verify_snapshot(config: dict[str, object]) -> None:
    ...
    if mismatch:
        raise ValueError(f"V2.7.4 changed after seventeenth holdout declaration: {mismatch}")
...
def documents() -> dict[str, object]:
    ...
    for source in config["sources"]:
        ...
        if sha256_file(path) != source["sha256"]:
            raise ValueError(f"V2.7.4 seventeenth holdout source hash mismatch: {ticker}")
...
def _generate(document: object):
    return augment_candidates_v274(document, generate_v273(document))
...
def candidate_pool() -> pd.DataFrame:
    ...
    return build_candidate_pool(
        ...
        route_blocks=route_document_blocks_v274,
    )
...
def candidate_blocks() -> pd.DataFrame:
    ...
    return select_candidate_blocks(
        pool, config, benchmark_label="V2.7.4 seventeenth holdout"
    )
