...
def evaluate_eighth_holdout():
    ...
    summary.loc[:, "version"] = "PLATFORM_V2_6_5_EIGHTH_DISJOINT_HOLDOUT"
    summary.loc[:, "status"] = (
        "V265_RESEARCH_BENCHMARK_PASS" if passed else "HOLD_RESEARCH_UNFROZEN"
    )
    return detail, summary, ladder, rejections


def main() -> int:
    detail, summary, ladder, rejections = evaluate_eighth_holdout()
    write_csv_artifacts(
        OUTPUT,
        {
            "eighth_holdout_evaluation": detail,
            "eighth_holdout_summary": summary,
            "eighth_holdout_ladder": ladder,
            "eighth_holdout_rejection_taxonomy": rejections,
        },
    )
    metadata = {
        "version": "PLATFORM_V2_6_5_EIGHTH_DISJOINT_HOLDOUT",
    ...
    (OUTPUT / "eighth_holdout_metadata.json").write_text(
