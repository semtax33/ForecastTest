...
from equity_platform.text_ie.v288.router import route_document_blocks_v288
from equity_platform.text_ie.v288.semantics import augment_candidates_v288
from scripts.architecture.parser_holdout import build_candidate_pool, select_candidate_blocks
import scripts.architecture.v287_thirtieth_holdout_candidates as parent


CONFIG = PROJECT_ROOT / "configs/certification/platform_v288_thirty_first_holdout.toml"
OUTPUT = PROJECT_ROOT / "output/platform_v2_8_8_local_role_binder"
CANDIDATES = OUTPUT / "thirty_first_holdout_candidates.csv"
ARCANA_IR = Path("D:/Programming/python_example/Arcana/data-lake/bronze/sec/fillings/ir")
V288_SNAPSHOT = {
    "semantics": PROJECT_ROOT / "equity_platform/text_ie/v288/semantics.py",
    "runtime": PROJECT_ROOT / "equity_platform/text_ie/v288/runtime.py",
    "router": PROJECT_ROOT / "equity_platform/text_ie/v288/router.py",
    "init": PROJECT_ROOT / "equity_platform/text_ie/v288/__init__.py",
    "rules": PROJECT_ROOT / "configs/parser_rules/text_ie/v288_local_role_binder.arc",
    "test": PROJECT_ROOT / "tests/test_spacy_local_role_binder_v288.py",
}
...
def verify_snapshot(config: dict[str, object]) -> None:
    ...
    expected = dict(config["v288_code_snapshot_sha256"])
    mismatch = {
        name: sha256_file(path)
        for name, path in V288_SNAPSHOT.items()
        if sha256_file(path) != expected[name]
    }
    if mismatch:
        raise ValueError(f"V2.8.8 changed after thirty-first declaration: {mismatch}")
...
def documents() -> dict[str, object]:
    ...
    for source in config["sources"]:
        ...
        if sha256_file(path) != source["sha256"]:
            raise ValueError(f"V2.8.8 thirty-first source hash mismatch: {ticker}")
...
def _generate(document: object):
    return augment_candidates_v288(document, tuple(parent._generate(document)))
...
def candidate_pool() -> pd.DataFrame:
    ...
    return build_candidate_pool(
        ...
        route_blocks=route_document_blocks_v288,
    )
...
def candidate_blocks() -> pd.DataFrame:
    ...
    return select_candidate_blocks(
        pool,
        config,
        benchmark_label="V2.8.8 thirty-first holdout",
    )
