...
def evaluate_thirtieth_holdout():
    with _frozen_inputs():
    ...
    summary.loc[:, "version"] = "PLATFORM_V2_8_7_THIRTIETH_DISJOINT_HOLDOUT"
    summary.loc[:, "status"] = (
        "V287_INDEPENDENT_RESEARCH_BENCHMARK_PASS"
        if passed
...
def main() -> int:
    detail, summary, ladder, rejections = evaluate_thirtieth_holdout()
    write_csv_artifacts(OUTPUT, {
        "thirtieth_holdout_evaluation": detail,
        "thirtieth_holdout_summary": summary,
        "thirtieth_holdout_ladder": ladder,
        "thirtieth_holdout_rejection_taxonomy": rejections,
    })
    metadata = {
        "version": "PLATFORM_V2_8_7_THIRTIETH_DISJOINT_HOLDOUT",
    ...
    (OUTPUT / "thirtieth_holdout_metadata.json").write_text(
