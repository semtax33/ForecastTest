...
from equity_platform.text_ie.v277.router import route_document_blocks_v277
...
CONFIG = PROJECT_ROOT / "configs/certification/platform_v277_twentieth_holdout.toml"
OUTPUT = PROJECT_ROOT / "output/platform_v2_7_7_spacy_semantic_ownership"
CANDIDATES = OUTPUT / "twentieth_holdout_candidates.csv"
...
def verify_snapshot(config: dict[str, object]) -> None:
    ...
    if mismatch:
        raise ValueError(f"V2.7.7 changed after twentieth holdout declaration: {mismatch}")
...
def documents() -> dict[str, object]:
    ...
    for source in config["sources"]:
        ...
        if sha256_file(path) != source["sha256"]:
            raise ValueError(f"V2.7.7 twentieth holdout source hash mismatch: {ticker}")
...
def candidate_pool() -> pd.DataFrame:
    ...
    return build_candidate_pool(
        config=config,
        documents=documents(),
        generate_candidates=generate_v276,
        route_blocks=route_document_blocks_v277,
    )
...
def candidate_blocks() -> pd.DataFrame:
    ...
    return select_candidate_blocks(
        pool, config, benchmark_label="V2.7.7 twentieth holdout"
    )
