...
def evaluate_twenty_sixth_holdout():
    with _frozen_inputs():
    ...
    summary.loc[:, "version"] = "PLATFORM_V2_8_3_TWENTY_SIXTH_DISJOINT_HOLDOUT"
    summary.loc[:, "status"] = (
        "V283_INDEPENDENT_RESEARCH_BENCHMARK_PASS"
        if passed
...
def main() -> int:
    detail, summary, ladder, rejections = evaluate_twenty_sixth_holdout()
    write_csv_artifacts(
        OUTPUT,
        {
            "twenty_sixth_holdout_evaluation": detail,
            "twenty_sixth_holdout_summary": summary,
            "twenty_sixth_holdout_ladder": ladder,
            "twenty_sixth_holdout_rejection_taxonomy": rejections,
        },
    )
    metadata = {
        "version": "PLATFORM_V2_8_3_TWENTY_SIXTH_DISJOINT_HOLDOUT",
    ...
    (OUTPUT / "twenty_sixth_holdout_metadata.json").write_text(
